            ...
            if (selectors.hasOwnProperty(newSelector)) {
              ...
              if (newSelector.indexOf("@") === 0) {
                ...
                extractRules(
                  [selectorValue_1],
                  rules,
                  expandSelector(newSelector, currentSelector)
                );
              }
            }
          };
...
export function serializeRuleEntries(options, ruleEntries) {
  ...
  // Apply punctuation.
  for (var j = 1; j < allEntries.length; j += 4) {
    allEntries.splice(j, 1, ":", allEntries[j], ";");
  }
    ...
    for (var i = 0; i < rulesToInsert.length; i += 2) {
      ...
      if (rules) {
        ...
        selector = selector.replace(
          /&/g,
          repeatString("." + registration.className, specificityMultiplier)
        );
