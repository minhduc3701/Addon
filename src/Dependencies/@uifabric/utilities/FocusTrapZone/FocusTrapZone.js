...
var FocusTrapZone = /** @class */ (function (_super) {
  ...
  function FocusTrapZone(props) {
    ...
    _this._forceClickInTrap = function (ev) {
      ...
      if (
        ...
        if (
          clickedElement &&
          !elementContains(_this._root.current, clickedElement)
        ) {
          _this.focus();
          _this._hasFocus = true; // set focus here since we stop event propagation
          ev.preventDefault();
  ...
  FocusTrapZone.prototype.componentDidMount = function () {
    ...
    if (
      !this.props.disabled &&
      this._root.current &&
      this.props.enableAriaHiddenSiblings
    ) {
  ...
  FocusTrapZone.prototype.componentDidUpdate = function (prevProps) {
    var prevForceFocusInsideTrap =
      prevProps.forceFocusInsideTrap !== undefined
        ? prevProps.forceFocusInsideTrap
        : true;
    var newForceFocusInsideTrap =
      this.props.forceFocusInsideTrap !== undefined
        ? this.props.forceFocusInsideTrap
        : true;
    var prevDisabled =
      prevProps.disabled !== undefined ? prevProps.disabled : false;
    var newDisabled =
  ...
  FocusTrapZone.prototype.focus = function () {
    var _a = this.props,
      focusPreviouslyFocusedInnerElement =
        _a.focusPreviouslyFocusedInnerElement,
    ...
    if (this._root.current) {
      if (focusSelector) {
        _firstFocusableChild = this._root.current.querySelector(
          "." + focusSelector
        );
      }
  ...
  FocusTrapZone.prototype._returnFocusToInitiator = function () {
    ...
    FocusTrapZone._focusStack = FocusTrapZone._focusStack.filter(function (
      value
    ) {
      return _this !== value;
    });
    ...
    if (
      ...
      this._previouslyFocusedElementOutsideTrapZone &&
      typeof this._previouslyFocusedElementOutsideTrapZone.focus ===
        "function" &&
      (elementContains(this._root.current, activeElement) ||
  ...
  FocusTrapZone.prototype._updateEventHandlers = function (newProps) {
    ...
    if (forceFocusInsideTrap && !this._disposeFocusHandler) {
      this._disposeFocusHandler = on(
        window,
        "focus",
        this._forceFocusInTrap,
        true
      );
  ...
  FocusTrapZone.prototype._isBumper = function (element) {
    return (
      element === this._firstBumper.current ||
      element === this._lastBumper.current
    );
  };
