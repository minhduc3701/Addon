...
export function useCustomizationSettings(properties, scopeName, localSettings) {
  var _a = React.useState(
      Customizations.getSettings(properties, scopeName, localSettings)
    ),
    settings = _a[0],
    setSettings = _a[1];
  var onCustomizationChange = React.useCallback(
    function () {
      var globalSettings = Customizations.getSettings(
        properties,
        scopeName,
        localSettings
      );
    ...
    [properties, scopeName, localSettings, setSettings]
  );
  React.useEffect(
    function () {
