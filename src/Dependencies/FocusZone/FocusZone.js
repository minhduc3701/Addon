...
var FocusZone = /** @class */ (function (_super) {
  ...
  function FocusZone(props) {
    ...
    _this._onKeyDown = function (ev) {
      ...
      if (
        _this._getDocument().activeElement === _this._root.current &&
        _this._isInnerZone
      ) {
      ...
      if (
        ...
        if (innerZone) {
          ...
          if (
            !_this.focusElement(
              getNextElement(ev.target, ev.target.firstChild, true)
            )
          ) {
            return;
          }
        ...
        switch (ev.which) {
          ...
          case KeyCodes.tab:
            if (
              ...
              (_this.props.handleTabKey ===
                FocusZoneTabbableElements.inputOnly &&
          ...
          case KeyCodes.home:
            ...
            if (
              ...
              firstChild &&
              _this.focusElement(
                getNextElement(_this._root.current, firstChild, true)
              )
            ) {
              break;
            }
    ...
    _this._getHorizontalDistanceFromCenter = function (
      ...
      var isValidCandidateOnpagingDown =
  ...
  FocusZone.prototype.componentDidMount = function () {
    ...
    if (root) {
      ...
      if (
        ...
        typeof this.props.defaultTabbableElement === "string"
      ) {
        this._activeElement = this._getDocument().querySelector(
          this.props.defaultTabbableElement
        );
        // tslint:disable-next-line:deprecation
  ...
  FocusZone.prototype.componentWillUnmount = function () {
    ...
    if (!this._isInnerZone) {
      ...
      if (this._windowElement && _outerZones.size === 0) {
        this._windowElement.removeEventListener(
          "keydown",
          this._onKeyDownCapture,
          true
        );
    ...
    if (this._root.current) {
      if (
        ...
        if (ownerZoneElement !== this._root.current) {
          var ownerZone =
            _allInstances[
              ownerZoneElement.getAttribute(FOCUSZONE_ID_ATTRIBUTE)
            ];
        ...
        return this.focusElement(
          getNextElement(this._root.current, firstChild, true)
        );
      }
    }
    return false;
  ...
  FocusZone.prototype.focusLast = function () {
    if (this._root.current) {
      ...
      return this.focusElement(
        getPreviousElement(this._root.current, lastChild, true, true, true)
      );
    }
    return false;
  };
    ...
    if (doc) {
      ...
      if (focusedElement !== root) {
        ...
        this._lastIndexPath = shouldRestoreFocus
          ? getElementIndexPath(root, focusedElement)
          : undefined;
      }
    }
  };
  ...
  FocusZone.prototype._moveFocusLeft = function () {
    var _this = this;
    var shouldWrap = this._shouldWrapFocus(
      this._activeElement,
      NO_HORIZONTAL_WRAP
    );
    if (
  ...
  FocusZone.prototype._moveFocusPaging = function (isForward, useDefaultWrap) {
    ...
    do {
      ...
      if (element) {
        ...
        var targetRectBottom = Math.floor(targetRect.bottom);
        var activeRectTop = Math.floor(activeRect.top);
        var elementDistance = this._getHorizontalDistanceFromCenter(
          isForward,
          activeRect,
          targetRect
        );
        var isElementPassedPageSizeOnPagingDown =
